...
rawuncertgraph<-function(uncertobj,multcutobj,xlab="Optical Density",scalecontrol=F,setcolor=c(
  "green4",          #1
  "turquoise3",      #2
  "royalblue2",      #3
  "darkorchid2",     #4
  ...
  if (scalecontrol==F) {
    ...
    datasorted<-uncertobj$datawithids[order(uncertobj$datawithids$data),] 
    maxless5<-datasorted$data[length(datasorted$data)-5]
    plot(uncertobj$v, uncertobj$mcluncert, main=paste("Uncertainty Plot with Possible Cutpoints for", uncertobj$desc$dist, "with", uncertobj$desc$ncomp, "components", sep = " "), type="l", ylim=c(0,1), xlim=c(round(min(uncertobj$datawithids$data),digits=1),maxless5), xlab=xlab, ylab="Uncertainty", sub="Note: This version of the graph is scale controlled and extreme values may be omitted.")
  }
  col<-setcolor
...
rawdistgraph<-function(pickobj,multcutobj,scalecontrol=F,xlab="Optical Density",setbreaks=100,setcolor=c(
  "green4",          #1
  "turquoise3",      #2
  "royalblue2",      #3
  "darkorchid2",     #4
...
rawhistcuts<-function(uncertobj,multcutobj,xlab="Optical Density",scalecontrol=F,setbreaks=250,setcolor=c(
  "green4",          #1
  "turquoise3",      #2
  "royalblue2",      #3
  "darkorchid2",     #4
  ...
  if (scalecontrol==F) {
    ...
    dataless5<- datasorted[1:(length(datasorted$data)-5),]
    maxless5<-max(dataless5$data)
    xlim=c(round(min(uncertobj$datawithids$data),digits=1),maxless5)
    hist<-hist(dataless5$data, breaks=setbreaks, plot=F)
  }
  ncomp<-uncertobj$desc$ncomp
  ...
  if (scalecontrol==F) {
    ...
    plot(hist, col=colors, xlab=xlab, ylab="Density", main=paste("Classification by", uncertobj$desc$dist, "with", ncomp, "components", sep = " "), freq=F, lty="blank", xlim=xlim, sub="Note: This version of the graph is scale controlled and extreme values may be omitted.")
  }
  legtxt<-vector("character", ncomp)
  colors<-vector("character", ncomp)
  for (i in 1:ncomp){
